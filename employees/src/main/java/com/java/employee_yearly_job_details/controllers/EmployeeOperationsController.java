...
public class EmployeeOperationsController {
    ...
    public ResponseEntity<EmployeeResponseBean> fetchEmployees() {
        ArrayList<Employees> empList = (ArrayList<Employees>) employeeRepository.findAll();
        EmployeeResponseBean erBean = new EmployeeResponseBean();
        if (!empList.isEmpty()) {
    ...
    public ResponseEntity<EmployeeResponseBean> addEmployees(@RequestBody EmployeeDetailBean empBean) {
        EmployeeResponseBean erBean = new EmployeeResponseBean();
    ...
    public ResponseEntity<EmployeeResponseBean> searchEmployees(@RequestBody EmployeeDetailBean empBean) {
        EmployeeResponseBean erBean = new EmployeeResponseBean();
        Optional<Employees> index = employeeService.searchData(empBean.getEmp().getEmployeeID());
        if (index.isPresent()) {
            erBean.setEmp(index.get());
            erBean.setResponseMessage("Successfully found Employee Id " + empBean.getEmp().getEmployeeID() + " data records");
            erBean.setResponseStatusCode(HttpStatus.OK);
        } else {
    ...
    public ResponseEntity<EmployeeResponseBean> updateEmployees(@RequestBody EmployeeDetailBean empBean) {
        EmployeeResponseBean erBean = new EmployeeResponseBean();
    ...
    public ResponseEntity<EmployeeResponseBean> deleteEmployees(@RequestBody EmployeeDetailBean empBean) {
        EmployeeResponseBean erBean = new EmployeeResponseBean();
